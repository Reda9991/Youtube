...
-- تابع لتحميل الفيديو أو الصوت
function download_media(url, format, output)
    local cmd = ""
    if format == "video" then
        cmd = string.format("youtube-dl -f mp4 -o %s %s", output, url)
    else
        cmd = string.format("youtube-dl --extract-audio --audio-format mp3 -o %s %s", output, url)
    end
